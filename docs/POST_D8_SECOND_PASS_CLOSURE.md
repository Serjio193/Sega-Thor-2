...
|---|---|---|---|---|---|---|---|
| **M-01** | `AJBats/SaturnAutoRE` | Low-level IPC harness (`MednafenBot`) | `ADOPT_PARTIAL` | `HIGH` | `HIGH` | `ACTIVE_INFRASTRUCTURE` | Operational (D1/V-01, V-07C) |
| **M-02** | `AJBats/SaturnAutoRE` | NOP / mutation fault injection | `ADOPT_PARTIAL` | `HIGH` | `HIGH` | `NEGATIVE_CONTROL_HARNESS` | Operational (`test_mutation_harness`) |
| **M-03** | `AJBats/SaturnAutoRE` | Autonomous loop / candidate scanner | `DEFER` | `LOW` | `MEDIUM` | `DISCOVERY_METHOD` | Blocked at `D9` (Multi-block CFG) |
| **M-04** | `AJBats/SaturnAutoRE` | Function boundary heuristics | `DEFER` | `LOW` | `MEDIUM` | `CANDIDATE_BOUNDARY_PROPOSAL` | Blocked at `D12` (Structural Recovery) |
| **M-05** | `saturn-daytona-cce-re` / Ghidra | RAM mapping & Ghidra 11.2.1 data types | `ADOPT_PARTIAL` | `CONFIRMED` | `HIGH` | `ACTIVE_INFRASTRUCTURE` | Operational (D2/V-02a, static RE) |
| **M-06** | `saturn-daytona-cce-re` | Linker script / section reconstruction | `DEFER` | `HYPOTHESIS` | `HIGH` | `BINARY_TOPOLOGY_REFERENCE` | Blocked at `D12 & D13` (Provenance) |
| **M-07A** | `SaturnRecomp` | SH-2 decoder & semantic reference | `ADOPT_PARTIAL` | `MEDIUM` | `HIGH` | `DECODER_AND_SEMANTIC_REFERENCE` | Operational (`test_m07_reference.py`) |
| **M-07B** | `SaturnRecomp` | Public AOT translation emitter / codegen | `NOT_PRESENT_AT_PIN` | `N/A` | `N/A` | `NONE` | Absent upstream at commit `26c9715` |
| **M-08** | `SaturnRecomp` / Emulators | Wholesale Saturn runtime / emulator fallback | `REJECT_MAINTAINED` | `N/A` | `LOW` | `NONE` | Architectural Constraint (ADR D-006) |
| **M-09** | Sega Saturn SDK / SGI | Header structures & peripheral layouts | `DEFER` | `MEDIUM` | `HIGH` | `SEMANTIC_TYPE_CANDIDATES` | Blocked at `D15` (HW Subsystems) |
| **M-10** | Historical Toolchains | Compiler fingerprinting (GCC 2.7 / Cygnus) | `DEFER` | `LOW` | `HIGH` | `CFG_RECONSTRUCTION_ACCELERATOR` | Blocked at `D12` (Structural Recovery) |
...
- **External Project / Method**: `AJBats/SaturnAutoRE` / `AJBats/mednafen-saturn-debug` low-level IPC bot harness.
- **Pinned Artifact / Commit**: Commit `155426661b7ac3152e2c93a98da60ac33002b908`.
...
- **Evidence Strength**: `HIGH` (Direct, reproducible dynamic execution against Mednafen emulator oracle).
- **Workflow Utility**: `HIGH` (Core automation foundation for differential verification).
- **Final Disposition**: `ADOPT_PARTIAL`
- **Pipeline Role**: `ACTIVE_INFRASTRUCTURE` (Proof & Differential Automation).
- **Testability NOW**: Fully operational and testable now (`test_v07c_timing_oracle`).
- **Prerequisite / Future Gate**: None (Operational).
- **Evidence Path**: `tools/runner/saturn_oracle.cpp`, `tools/runner/mednafen_bot.py`, `docs/DECISIONS.md#D-010`.
- **Closure Verdict**: **PASS / OPERATIONAL**.

---

### M-02 — SaturnAutoRE NOP / Byte-Mutation Fault Injection
- **External Project / Method**: `AJBats/SaturnAutoRE` automated NOP and byte mutation fault-injection technique.
- **Pinned Artifact / Commit**: Commit `ca88cf23b2c6d7d51944daaa2d41571214041a99`.
...
- **Evidence Strength**: `HIGH` (for negative controls, falsification, and harness boundary validation; not positive equivalence proof).
- **Workflow Utility**: `HIGH` (Automated negative regression testing preventing false positives in candidate matching).
- **Final Disposition**: `ADOPT_PARTIAL`
- **Pipeline Role**: `NEGATIVE_CONTROL_HARNESS` / `FAULT_INJECTION_TESTING`.
- **Testability NOW**: Fully operational and testable now (`test_mutation_harness`).
- **Prerequisite / Future Gate**: None (Operational).
- **Evidence Path**: `src/recomp/mutation_harness.cpp`, `tools/recomp/mutation_harness.py`, `tests/recomp/test_mutation_harness.cpp`, `workstreams/POST-D8-M02-mutation-re/experiment_evidence.md`.
...
- **Evidence Strength**: `LOW` (Autonomous heuristics cannot establish ground truth).
...
- **Prerequisite / Future Gate**: Blocked until multi-block flow and indirect branch resolution are designed at `D9 (Multi-Block Expansion Architecture)`.
- **Evidence Path**: `docs/POST_D8_SECOND_PASS_PLAN.md`, `workstreams/POST-D8-M02-mutation-re/experiment_evidence.md`.
...
- **Evidence Strength**: `LOW` (Boundary heuristics are hypotheses prone to error on shared tails and switch tables).
- **Workflow Utility**: `MEDIUM` (Candidate boundary generation for human review).
- **Final Disposition**: `DEFER`
- **Pipeline Role**: `CANDIDATE_BOUNDARY_PROPOSAL`.
- **Testability NOW**: Not testable now; current execution model is single-block.
- **Prerequisite / Future Gate**: Blocked until call/return intra-procedural CFG and stack analysis exist at `D12 (Structural Recovery)`.
- **Evidence Path**: `docs/POST_D8_SECOND_PASS_PLAN.md`, `AGENTS.md` (Function boundaries rule).
- **Closure Verdict**: **PREREQUISITE_BLOCKED_AT_D12**.

---

### M-05 — saturn-daytona-cce-re RAM Mapping & Ghidra 11.2.1 Data Types
- **External Project / Method**: `saturn-daytona-cce-re` module RAM mapping via SHA-256 / Ghidra 11.2.1 data type archives.
- **Pinned Artifact / Commit**: Ghidra 11.2.1 + SGL/SBL/Thor 2 GDT data types; `saturn-daytona-cce-re` RAM hash verification method.
- **First-Pass Status**: `ADOPT_PARTIAL` under [ADR D-011](DECISIONS.md#D-011).
- **Second-Pass Concrete Evidence**: Module RAM layout and entry hashes verified dynamically under D2 (`V-02a`). Ghidra 11.2.1 analysis database with imported Sega header data types is utilized for manual, human-directed static exploration (strictly non-headless, zero imported retail bytes committed to repository).
- **Evidence Strength**: `CONFIRMED` (SHA-256 memory hashes match retail disc loads).
- **Workflow Utility**: `HIGH` (Ensures binary integrity and accelerates static reverse engineering).
- **Final Disposition**: `ADOPT_PARTIAL`
- **Pipeline Role**: `ACTIVE_INFRASTRUCTURE` (Binary Integrity & Static Exploration).
- **Testability NOW**: Fully operational and testable now (`test_v02a_ram_map`).
- **Prerequisite / Future Gate**: None (Operational).
- **Evidence Path**: `tests/recomp/test_v02a_ram_map.cpp`, `docs/DECISIONS.md#D-011`.
- **Closure Verdict**: **PASS / OPERATIONAL**.
...
- **Pinned Artifact / Commit**: Daytona CCE reverse-engineering repository linker scripts.
- **First-Pass Status**: `DEFERRED`.
...
- **Workflow Utility**: `HIGH` (Essential for final pure native linking).
- **Final Disposition**: `DEFER`
- **Pipeline Role**: `BINARY_TOPOLOGY_REFERENCE`.
- **Testability NOW**: Not testable now; requires global cross-module reference topology.
- **Prerequisite / Future Gate**: Blocked until recovered function ASTs and recovered types exist at `D12 (Structural Recovery)` and `D13 (Guest-Address/Type Provenance)`.
...
- **Testability NOW**: Fully operational and testable now (`test_m07_reference.py`).
- **Prerequisite / Future Gate**: None (Operational).
- **Evidence Path**: `workstreams/POST-D8-M07-saturnrecomp/reference_vectors.json`, `tools/recomp/saturnrecomp_adapter.py`, `tests/recomp/test_m07_reference.py`, `workstreams/POST-D8-M07-saturnrecomp/experiment_evidence.md`.
- **Closure Verdict**: **PASS / ADOPTED**.
...
- **Final Disposition**: `NOT_PRESENT_AT_PIN` (`REJECT_AT_PIN` / `DEFER`).
- **Pipeline Role**: `NONE`.
...
- **Workflow Utility**: `LOW` (Violates core mission).
- **Final Disposition**: `REJECT_MAINTAINED`
- **Pipeline Role**: `NONE` (Strictly prohibited in production by ADR D-006).
- **Testability NOW**: N/A.
- **Prerequisite / Future Gate**: None (Permanent architectural constraint).
- **Evidence Path**: `docs/DECISIONS.md#D-006`, `AGENTS.md` Rule 15.
...
- **Pinned Artifact / Commit**: Sega Saturn SDK documentation and historical header corpora.
- **First-Pass Status**: `REFERENCE_ONLY`.
- **Second-Pass Concrete Evidence**: Evaluated under post-D8 audit. Official headers define register structures for VDP1, VDP2, SCU, and SMPC. At D8, execution is strictly confined to basic block 0 (`bb_06004000`), which accesses only CPU registers and general RAM, with zero MMIO or peripheral interaction.
- **Evidence Strength**: `MEDIUM` (Historical reference; Ancient may have utilized custom driver code or direct MMIO writes).
- **Workflow Utility**: `HIGH` (Accelerates peripheral struct definition).
- **Final Disposition**: `DEFER`
...
- **Pinned Artifact / Commit**: Hitachi/Cygnus GCC 2.7-96q1 compiler suite.
- **First-Pass Status**: `HEURISTIC`.
- **Second-Pass Concrete Evidence**: Evaluated under post-D8 audit. Compiler fingerprinting matches register allocation conventions, stack frame layout, and instruction scheduling patterns. At D8, instruction semantics are verified mechanically at basic block granularity. Semantic decompilation matching requires recovered multi-block function CFGs.
- **Evidence Strength**: `LOW` (Suggests toolchain patterns, not verified gameplay semantics).
...
- **Testability NOW**: Not testable now; requires multi-block subroutines and decompiled ASTs.
- **Prerequisite / Future Gate**: Blocked until multi-block subroutine CFGs and decompilation pipelines exist at `D12 (Structural Recovery)`.
- **Evidence Path**: `docs/RE_TOOLCHAIN_GUIDE.md`, `docs/POST_D8_SECOND_PASS_PLAN.md`.
- **Closure Verdict**: **PREREQUISITE_BLOCKED_AT_D12**.
...
1. **Adopted Operational Infrastructure**:
   ...
   - `M-05`: Active RAM hash integrity verification and Ghidra data type analysis.
