...
- **D8 / T2-D8.1 / V-07C — Authoritative Native Promotion Proof**: **BOUNDED_PROOF for bb_06004000 / V-07C: PASS** (decision D-014; reusable `NativeDispatcher` with mandatory pre-execution shadow qualification and C ABI bridge implemented; live authoritative override executed in pinned Mednafen debug oracle on cold boot [Run A]; bit-identical reproduction confirmed [Run B]; live interpreter retired 0 instructions in replaced block [retirements_in_interval = 0]; continuation cleanly verified to `0x06004280` matching interpreter baseline across all 23 registers with 0 divergences and 0 cycle drift [exact delta = 0 cycles at cycle 307090585, eliminating earlier +14 cycle cache-drift via `CPU[0].MRFP/MWFP` memory routing]; 100% fail-closed fallback proven under memory corruption; mandatory post-D8 second-pass plan established under ADR D-012).
...
- **D9 / T2-D9.1 / T2-D9.2 — Indirect Control-Flow Handling**: **READY_FOR_BOUNDED_TEST (D9.1: PASS, D9.2: PASS)** (canonical architecture plan in `docs/D9_INDIRECT_CONTROL_FLOW_PLAN.md`; `JSR @Rn` exact decode and execution semantics implemented; candidate block `bb_06004280` [`0x06004280..0x06004288`, 10 bytes, SHA-256 `8879cbe1...`] qualified in D3/D4/D5; generic `BlockExitDescriptor` and `ResolvedBlockExit` implemented, cleanly separating static CFG structure from runtime post-state target resolution; `NativeDispatcher` target handling generalized with hardcoded `target_pc` removed while preserving D8 behavior; declarative `BlockMemoryContract` implemented distinguishing `STATIC_ADDRESS` and `REGISTER_AT_EXECUTION` with exact 3 dependencies proven for `bb_06004000` and `bb_06004280`; candidate machine-readable metadata established in `candidate_06004280.json`; timing reconciled to 21 cycles [19 cycles to delay slot entry]; automated validator `test_d9_plan.py` passing with 19 negative controls; M-03 re-entry gated at `D9.4`).

Active next capability: **D9.3 Mechanical JSR Block Generation + Isolated Shadow Qualification for bb_06004280**.

Note: Basic block `bb_06004000` (`0x06004000..0x0600400A`) has complete decode, L0 semantic, code ownership, CFG, identity guard, minimum event safety, mechanical C++ transition proof, shadow checker validation, and authoritative native promotion proof. Candidate `bb_06004280` has complete decode, L0 semantic, code ownership, and CFG qualification under D3/D4/D5. D9 as a whole remains `READY_FOR_BOUNDED_TEST` until generic dispatch, shadow qualification, and native override are proven. D8/D9 are NOT whole-system recompiler completion.
