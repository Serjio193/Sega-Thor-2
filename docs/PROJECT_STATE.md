...
- **D9 / T2-D9.1 — Indirect Control-Flow Handling**: **READY_FOR_BOUNDED_TEST (D9.1: PASS)** (canonical architecture plan in `docs/D9_INDIRECT_CONTROL_FLOW_PLAN.md`; `JSR @Rn` exact decode and execution semantics implemented; candidate block `bb_06004280` [`0x06004280..0x06004288`, 10 bytes, SHA-256 `8879cbe1...`] qualified in D3/D4/D5; timing reconciled to 21 cycles [entry 316309168, delay-slot entry 316309187, target entry 316309189]; two independent cold boots in Mednafen oracle confirmed 100% bit-identical execution; invalid `V-09A` label eliminated; automated validator `test_d9_plan.py` passing with 11 negative controls; M-03 re-entry gated at `D9.4`).

Active next capability: **D9.2 Generic Dynamic-Exit Representation & Declarative Memory Descriptors**.
