...
- **FULL_ASM_GAME_GATE — Full Saturn Disc Game Boot & Gameplay Verification**: **PASS** (all 3 SH-2 executable modules `0TH2.BIN`, `TH2.LOW`, and `SET07.BIN` reassembled byte-exact from assembly sources with pinned GNU `binutils-sh-elf 2.40+2`; spliced simultaneously into private disc image `thor2_full_rebuilt.bin` matching canonical retail disc SHA-256 `fe11d2fb...`; Mednafen cold-boot runtime parity verification in pure interpreter mode verified across all 6 architectural checkpoints [`entry_06004000`, `branch_target_06004012`, `checkpoint_06004280_occ0`, `checkpoint_06004280_occ1`, `checkpoint_0600A0F8_load_th2_low`, `checkpoint_002E9910_th2_low_exec`] with 0 cycle divergence and 23/23 matching registers; integration test suite `test_full_game_disc` passing 100% across Windows MinGW and Linux WSL; 27/27 CTests pass; unblocks progressive native C++20 game subsystem recovery per ADR D-015).
- **D10 / T2-D10.1 — Timing/Interrupt/DMA Execution Boundaries**: **BOUNDED_PROOF / PASS** (formalized execution boundary taxonomy in `include/thor/recomp/block_timing.hpp` and `src/recomp/block_timing.cpp`: `ATOMIC_COMPUTATION`, `MMIO_SYNCHRONOUS`, `INTERRUPT_WINDOW`, `DMA_ASYNCHRONOUS`; implemented Saturn MMIO region recognition across SH-2 on-chip peripherals `0xFFFFFE00..0xFFFFFFFF` and B-Bus/VDP/SCU/SCSP mirrors `0x05800000..0x05FFFFFF` / `0x25800000..0x25FFFFFF`; implemented `classify_block_timing(...)` detecting hardware boundary crossings and enforcing synchronization barriers or interpreter fallbacks; verified with dedicated CTest suite `test_block_timing` across synthetic vectors, memory contracts, and bounded event metadata).
- **D11 / T2-D11.1 — Overlay/Generation Identity**: **BOUNDED_PROOF / PASS** (formalized multi-generation executable identity per AGENTS.md: `revision + CPU + module/overlay generation + guest address`; added generation tracking to `BlockIdentityDescriptor` and `GENERATION_MISMATCH` fail-closed rejection to `check_block_eligibility`; verified generation isolation between base modules [generation 0] and stage overlays [generation 7: `make_bb_060D8000_set07_descriptor()` at `0x060D8000`]; verified with `test_executable_identity`).
- **D12 / T2-D12.1 / T2-NAT-02 — Structural Recovery & Subsystem Function Boundary Demarcation**: **PASS** (formalized function boundary kinds and invariants in `include/thor/recomp/function_boundary.hpp` and `src/recomp/function_boundary.cpp`; demarcated canonical routines `sub_06004000_boot`, `sub_0600A0F8_load_file`, `sub_002E9910_engine_start`, `sub_060D8000_stage_overlay`; verified caller/callee adjacency and call graph queries; verified with dedicated CTest suite `test_function_boundary`; 29/29 CTests pass across Windows MinGW and Linux WSL).

Active next technical task: **D15 / T2-NAT-03 — Hardware Subsystem Contracts (VDP1, VDP2, SCSP)**.
