...
- **D9 / T2-D9.P0 — Indirect Control-Flow Handling**: **READY_FOR_BOUNDED_TEST** (canonical architecture plan in `docs/D9_INDIRECT_CONTROL_FLOW_PLAN.md`; dynamic exit model, generalized memory snapshot contract, fail-closed unknown target policy, and first candidate block `bb_06004280` [`0x06004280..0x06004288`, 10 bytes, SHA-256 `8879cbe1...`, cold-boot exit to `0x0600A0F8` in 19 cycles] qualified; automated validator `test_d9_plan.py` passing with 8 negative controls; M-03 re-entry gated at `D9.4`).

Active next capability: **D9.1 Candidate Opcode L0 Semantics & Block Qualification (JSR @Rn, PR update, delay slot)**.
