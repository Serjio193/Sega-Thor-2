...
def generate_assembly(
    ...
    cc = "|" if manifest.get("processor") == "MC68EC000" else "!"
    target_tool = (
        "GNU assembler (binutils-m68k-linux-gnu 2.42)"
        if manifest.get("processor") == "MC68EC000"
        else "GNU assembler (binutils-sh-elf 2.40+2)"
    )
    lines: List[str] = [
        f"{cc} ==============================================================================",
        f"{cc} The Story of Thor 2 (Saturn) — {module_name} Lossless Assembly Container",
        f"{cc} Revision: {manifest['revision']}",
        f"{cc} Base VMA: 0x{base_vma:08X} | Size: {module_size} bytes (0x{module_size:05X})",
        f"{cc} Toolchain target: {target_tool}",
        f"{cc} Mechanically generated by tools/asm/generate_full_module_asm.py",
        f"{cc} DO NOT COMMIT THIS FILE (Contains full commercial binary payload)",
        f"{cc} ==============================================================================",
        "",
        '    .section .text, "ax"',
    ]
    ...
    while offset < total_len:
        ...
        if offset in blocks_by_offset:
            ...
            lines.append(f"{cc} --- CONFIRMED_CODE: {blk_id} (VMA 0x{base_vma + offset:08X}) ---")
            insn_off = offset
            for insn in blk["instructions"]:
                emit_labels(insn_off)
                lines.append(f"    {insn['asm_line']:<32} {cc} {insn['comment']}")
                insn_off += insn.get("byte_length", 2)
            offset += blk["byte_length"]
            continue

        # 2. Check if manifest range with instructions starts here (e.g. M68K mnemonics)
        rng = ranges_by_start.get(offset)
        if rng and rng.get("assembly_representation") == "MNEMONIC_PROVEN" and "instructions" in rng:
            emit_labels(offset)
            end_off = rng["offset_end_exclusive"]
            lines.append(f"{cc} --- CONFIRMED_CODE (MNEMONIC_PROVEN: VMA 0x{base_vma + offset:08X}) ---")
            for insn in rng["instructions"]:
                lines.append(f"    {insn}")
            offset = end_off
            continue

        # 3. Emit labels at non-code offset
        emit_labels(offset)

        # 4. Check if RAW_CODE_PENDING_DECODE range
        if rng and rng.get("assembly_representation") == "RAW_CODE_PENDING_DECODE":
            end_off = rng["offset_end_exclusive"]
            lines.append(f"{cc} --- CONFIRMED_CODE (RAW_CODE_PENDING_DECODE: VMA 0x{base_vma + offset:08X}) ---")
            for cur_off in range(offset, end_off, 2):
                emit_labels(cur_off)
                b0 = module_bytes[cur_off]
                b1 = module_bytes[cur_off + 1]
                lines.append(f"    .byte   0x{b0:02X}, 0x{b1:02X} {cc} opcode 0x{(b0 << 8) | b1:04X}")
            offset = end_off
            continue
...
def generate_linker_script(manifest: Dict[str, Any], ir_data: Dict[str, Any], out_ld_path: str) -> None:
    ...
    out_fmt = "elf32-m68k" if is_m68k else "elf32-sh"
    out_arch = "m68k" if is_m68k else "sh:sh2"
...
def main() -> int:
    ...
    if not is_m68k and proven_ranges:
